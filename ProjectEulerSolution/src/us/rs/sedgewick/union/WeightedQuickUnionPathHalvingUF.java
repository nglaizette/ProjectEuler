...
/**
 * The <tt>WeightedQuickUnionPathHalvingUF</tt> class represents a union-find
 * data structure. It supports the <em>union</em> and <em>find</em> operations,
 * along with methods for determinig whether two objects are in the same
 * component and the total number of components.
 * <p>
 * This implementation uses weighted quick union (by size) with path compaction
 * (via halving). Initializing a data structure with <em>N</em> objects takes
 * linear time. Afterwards, <em>union</em>, <em>find</em>, and
 * <em>connected</em> take logarithmic time (in the worst case) and
 * <em>count</em> takes constant time. Moreover, the amortized time per
...
public class WeightedQuickUnionPathHalvingUF {
	...
	public static void main(String[] args) {
		int N = StdIn.readInt();
		WeightedQuickUnionPathHalvingUF uf = new WeightedQuickUnionPathHalvingUF(
		        N);
		while (!StdIn.isEmpty()) {
			int p = StdIn.readInt();
			int q = StdIn.readInt();
